...
- Define its constructor.
  ...
  - In a _private_ field `available_blocks`, it stores a set containing the numbers `0` to `block_count` (exclusive).
  - In a _private_ field `used_blocks`, it stores an empty set.
  - It stores the block size in a _private_ field `block_size`.
